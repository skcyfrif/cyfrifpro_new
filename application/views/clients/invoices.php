...
?>
      ...
      <div class="span12">
        <a href="<?php echo base_url();?>client/add_invoice" ><button class="btn btn-success btn-sm pull-right">+ Add</button></a><a href="javascript:void();" class="pull-right">&nbsp;  &nbsp;</a>
        <a href="<?php echo base_url();?>invoices/csv" ><button class="btn btn-success btn-sm pull-right">Import</button></a>
        <a href="<?php echo base_url();?>client/Spreadsheet_format_downlood" target= "-blank"><button class="btn btn-success btn-sm pull-right" style="margin-right:7px;">Downlood Format</button></a>
        <a href="<?php echo base_url();?>client/spreadsheet_export" target= "-blank"><button class="btn btn-success btn-sm pull-right" style="margin-right:7px;">Downlood file</button></a>
                ...
                $i=1; foreach($invoices as $key){ 
                    ...
                    <tr class="gradeX">
                      ...
                      <!--   <td> 
                        ...
                        <td colspan="2">
                          <?php //echo $key->paidFlag;?>
                            <select onclick="changeIt(this)" data-id="<?php echo $key->id;?>" data-amount="<?php echo $key->due;?>" data-tamount="<?php echo $key->received;?>">
                             <option value="due" <?php echo ($key->status == 'due') ? 'selected' : '';?> >--Select--</option>
                             <!--  <?php //if($key->paidFlag != 1){ ?><option value="due" <?php //echo ($key->status == 'due') ? 'selected' : '';?> >Due</option><?php //} ?> -->
                              <?php //if($key->paidFlag == 0){ ?><option value="received" <?php echo ($key->status == 'received') ? 'selected' : '';?> >Recieve</option><?php //} ?>
                              <?php //if($key->paidFlag == 1){ ?><option value="deposited" <?php echo ($key->status == 'deposited') ? 'selected' : '';?> >Deposit</option><?php //} ?>
                            </select>
                        ...
                        <td class="td-actions">
                            ...
                            <!--</a>-->
                            <a href="<?php echo base_url();?>client/invoice/<?php echo $key->id;?>" class="btn btn-xs btn-primary" target="_blank">
                                Invoice                                 
                            </a>

                            <button onclick="getHistory(this)" data-toggle="modal" data-target="#myModal2" href="#" class="btn btn-xs btn-default" data-invoice_id="<?php echo $key->id;?>">
                              History                                       
                            </button>

                            <?php if($key->preventEdit != 1){ ?>
                            <a href="<?php echo base_url();?>client/add_invoice/<?php echo $key->id;?>" class="btn btn-xs btn-info">
                                Edit                                       
                            </a>
                            <?php } ?>
